...
def fast_beam_search(args, models, batch_data,
    ...
    fast beam search
    '''
    src_emb = models['embedding'].get_embedding(src_text_rep)
    encoder_hy, hidden_encoder = models['encoder'](src_emb)
    hidden_decoder = models['encoder2decoder'](hidden_encoder)
    ...
    for j in range(max_len):
        ...
        trg_emb = models['embedding'].get_embedding(last_wd.view(-1, 1))
        ...
        if args.share_emb_weight:
            decoder_proj = models['decoder2proj'](trg_h_reshape)
            logits = models['embedding'].get_decode2vocab(decoder_proj)
        else:
            logits = models['decoder2vocab'](trg_h_reshape)
